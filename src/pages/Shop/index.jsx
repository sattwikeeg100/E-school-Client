        ...
        <div className="flex flex-row justify-start items-start w-full pl-14 pr-14 gap-10">
          <div className="md:hidden flex flex-col w-[31%] gap-[39px]">
              ...
              <div className="flex flex-col w-full gap-[15px]">
                {loading
                  ? Array.from({ length: 4 }).map((_, index) => (
                      <div key={index}>
                        <EduviShopPopularsCard loading={loading} />
                      </div>
                    ))
                  : bookdata.slice(0, 4).map((book) => (
                      <div key={book.isbn}>
                        <EduviShopPopularsCard
            ...
            <div className="flex flex-col items-start justify-start w-full gap-4">
              ...
              <div className="flex flex-col w-full gap-[15px]">
                {loading
                  ? // Render skeleton placeholders if loading is true
                    Array.from({ length: 3 }).map((_, index) => (
                      <div key={index}>
                        <EduviShopNewArrivalCard loading={loading} />
                      </div>
                    ))
                  : // Render actual content if loading is false
                    bookdata
                      .slice(Math.max(bookdata.length - 3, 0))
                      .map((book) => (
                        <div key={book.isbn}>
                          <EduviShopNewArrivalCard
                            imgsrc={book.image.url}
                            title={book.title}
                            author={book.author}
                            ratings={book.totalrating}
          ...
          <div className="flex flex-col items-center justify-start w-[65%] md:w-full gap-10">
            <div className="flex flex-col items-center justify-start w-full gap-[30px]">
              <div className="flex flex-row justify-start w-full gap-5 flex-wrap md:gap-2">
                <Button
                  color={
                    selectedCategory === "" ? "orange_200_01" : "white_A700"
                  }
                  className=" min-w-[120px] sm:text-sm md:w-[50px] rounded-[10px]"
                ...
                <Button
                  ...
                  className=" min-w-[120px] sm:text-sm md:w-[50px] rounded-[10px]"
                  onClick={() => handleCategorySelect("Coding")}
                >
                  Coding
                </Button>
                <Button
                  ...
                  className=" min-w-[120px] sm:text-sm md:w-[50px] rounded-[10px]"
                  onClick={() => handleCategorySelect("AI/ML/DS")}
                >
                  AI/ML/DS
                </Button>
                <Button
